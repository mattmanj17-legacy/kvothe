...
int*	g_mpISetCMarked;	// the number of marked elements in a set. numMarkedInS = M[s]

int*	g_aISetTouched;		// touched sets and count of touched sets
int		g_cSetTouched = 0;	// ... 
...
bool FCmpITranByLabel(int iTran0, int iTran1) 
{
...
inline void MarkReached(int iState) 
{ 	
...
int main() 
{
	...
	g_aISetTouched = new int[g_cTran + 1];
	g_mpISetCMarked = new int[g_cTran + 1];
	...
	if (g_cTran > 0) 
	{
		...
		std::sort(g_partTrans.m_aIElem, g_partTrans.m_aIElem + g_cTran, FCmpITranByLabel);
		...
		for (int cTran = 0; cTran < g_cTran; ++cTran) 
		{
			...
			if (g_mpITranLabel[iTran] != label) 
			{
				...
				g_mpISetCMarked[g_partTrans.m_cSet] = 0;
			}
